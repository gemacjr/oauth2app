...
export class LoginService {
  ...
  openIDConfiguration: Observable<OpenIDConfiguration>;
   baseUrl = 'https://swiftbeard.auth0.com';
   
 
  clientId = 'jB0J5KmttvbDuMhphQbGe9NWsRcXx5Re';
  ...
  constructor(private http: Http, private router: Router) {
    ...
    this.openIDConfiguration = this.getOpenIDConfiguration(this.baseUrl);

    console.log(' Open Id ' + this.getOpenIDConfiguration(this.baseUrl));
   ...
   * @return {Observable<R>}
   */
  public getOpenIDConfiguration(baseUrl: string): Observable<OpenIDConfiguration> {
    return this.http.get(`${baseUrl}/.well-known/openid-configuration`)
      .map(res => res.json())
   ...
   * @return {string}
   */
  private getAuthorizationRequestUrl(state: string, clientId: string): Observable<string> {
  ...
  authorize(route?: string[]) {
    ...
    this.getAuthorizationRequestUrl(state, clientId).subscribe(url => this.redirect(url));
  }
